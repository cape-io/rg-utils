...
const isLeatherName = propDo(
  'name',
  _.overEvery([_.isString, _.flow(_.toLower, includesLeather)]),
)
const leatherContent = ['cattle hide']
const contentIsLeather = _.overSome(_.map(_.includes, leatherContent))
export const isLeather = _.overSome([
  propDo('category', _.flow(_.toLower, oneOf(leatherNames))),
  propDo('patternId', patternIsLeather),
  propDo('content', contentIsLeather),
  isLeatherName,
])

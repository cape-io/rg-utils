import { LEATHER } from './cat-ids.js'
import {
  getCategory,
...
describe('isLeather', () => {
  test('valid leather name', () => {
    ...
    expect(isLeather({ name: 'Outdoor Leathr' })).toBe(true)
  })
...
describe('getCategory', () => {
  test('any remains any', () => {
    expect(getCategory({ category: 'any', itemId: '945026-01', colorId: '01' })).toBe('any')
  })
  test('invalid remains invalid', () => {
    expect(getCategory({ category: 'invalid' })).toBe('invalid')
    expect(getCategory({ })).toBe('invalid')
  })
  test('any cat but found leather', () => {
    expect(getCategory({ category: 'any', content: 'Cattle Hides' })).toBe(LEATHER)
  })

...
export function getCategoryColumns(info) {
  const category = getCategory(info.categoryId)
  if (!category) return []
...
// Get the columns for a display section.
export function getSectionColumns({ metaInfo, categoryId }) {
  const info = setCatId(metaInfo, categoryId)

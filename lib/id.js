...
// 5 bits is 32, 6 bits is 64, 7 bits is 127
const COLOR_BITS = 6
const CLR_MAX = 2 ** COLOR_BITS
...
export const isPatternNumber = (x) => _.isFinite(x) && x > 9 && x < PATTERN_MAX
...
export const numberToColorParts = _.cond([
  [isColorNum, (x) => ([x])],
  [isMixedColorNum, (x) => ([x >>> COLOR_BITS, x & (CLR_MAX - 1)])],
])
...
// 5 bits (127 max) for one number. 10 bits for two
...
export function itemPartsToNumber({ prefixNumber, patternNumber, colorNumber }) {
  if (colorNumber >= CLR_MAX) return null // Can't handle double numbers.
  // 2 24 6
  const preNum = prefixNumber * (1 << NUM_SHIFT)
  return preNum + ((patternNumber << COLOR_BITS) | colorNumber) - MIN_PATTERN
}
...
export function numberToItemParts(input) {
  ...
  return {
    input,
    ...prefixInfo,
    patternNumber: (num >>> COLOR_BITS) & (PATTERN_MAX - 1),

...
const isColorNum = _.flow(Number, (x) => x > 0 && x < 0x63) // 99 is max
const isMixedColorNum = (colorNum) => colorNum > 0x63 && colorNum < 0x6363
...
function archiveParts(input) {
  ...
  const prefixInfo = fromArchivePrefix(prefix)
  const colorNumbers = prefixInfo.colorIsRaw ? null : numberToColors(fromB32(remaining[0]))
  return {
    input,
    ...prefixInfo,

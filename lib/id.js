...
export const isPatternNumber = (x) => _.isFinite(x) && x >= PATTERN_MIN && x < PATTERN_MAX
// Single color value.
...
export function getPatternParts(pattern) {
  const patternNumber = Number(pattern)
  if (patternNumber < PATTERN_RAW_MAX) return { pattern, patternNumber }
...
export const isPatternPart = _.flow(
  ...
  }) => (!prefixInvalid && !!patternPrefix) || (!prefixInvalid && !!patternNumber),
)

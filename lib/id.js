...
export const idStrPrep = _.flow(
  _.toLower,
  _.trimChars(SEPARATOR),
  _.replace('/', SEPARATOR), // Note: not using underscore.
  _.replace('_', SEPARATOR),
  _.replace('|', SEPARATOR),
  _.replace(' ', SEPARATOR),
)
...
function getPatternErrors({ patternNumber, prefixInvalid, patternMinNumber }) {
  const errors = []
  if (prefixInvalid) {
    errors.push({ part: 'prefix', message: 'Invalid pattern prefix.' })
  }
  if (!patternNumber) {
...
export const getPatternIdInfo = _.flow(
  getPartsInfo,
...
export function isItemInfoValid(info) {
  const errors = []
  if (info.prefixInvalid) {
    errors.push('Invalid pattern prefix.')
  }
  if (info.patternMinNumber > info.patternNumber) {
    errors.push('Pattern number too small')
  }
  if (info.otherId) {
    errors.push('Item id has extra information at the end.')
  }
  return errors.length ? errors : undefined
}
...
export function itemIdObj({ remainingParts, ...info }) {
  ...
  const colorId = colorParts.length ? colorParts.map(toColorPart).join('|') : null
  const result = {
    ...info,
    colorId,
    otherId,
...
export function numberToPrefix(num) {
  ...
  if (num > 0x1F) return { prefixError: true }
...
// export function numberToPattern({ patternNumber, patternMinLength }) {
//   return padNumber(patternMinLength, patternNumber.toString())
// }
...
function archiveParts(input) {
  const [prefix, pattern, color] = splitOnSeparator(idStrPrep(input).toUpperCase())
  return {
    input,
    colorId: numberToColor(fromB32(color)),
    ...fromArchivePrefix(prefix),
...
export const fromArchiveId = _.flow(
  ...
  mergeFields(getInfoFromPrefix),
  setField('patternId', getPatternId),
  setField('itemId', partsToId),
  setField('archiveId', itemInfoToArchiveId),
)

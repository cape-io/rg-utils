import {
  ...
  getArchivePrefix,
  isColorIdNum, isItemId, idStrPrep, isPatternPart, isValidPattern, ispatternInt,
...
describe('idStrPrep', () => {
  ...
  test('other', () => {
    ...
    expect(idStrPrep('placemats-6')).toBe('placemats-6')
  })
})

describe('fixItemId', () => {
  test('turn raw to itemId', () => {
    expect(fixItemId('PF7006-03')).toBe('pf7006-03')
    expect(fixItemId('92705-05')).toBe('92705-05')
    expect(fixItemId('870011-01')).toBe('870011-01')
    expect(fixItemId('PF7006-0')).toBe(null)
  })
})

describe('isItemId', () => {
  test('valid ids', () => {
    expect(isItemId('PF7006-03')).toBe(true)
    expect(isItemId('92705-05')).toBe(true)
    expect(isItemId('870011-01')).toBe(true)
  })
  test('invalid ids', () => {
    expect(isItemId('PF7006')).toBe(false)
    expect(isItemId('PF7006-0')).toBe(false)
    // expect(isItemId('92705-97')).toBe(false) // @TODO
  })
})
describe('getArchivePrefix', () => {
  ...
  test('invalid patterns', () => {
    ...
    expect(getArchivePrefix(getItemIdInfo('placemats-6'))).toBe('AF') // why not AZ?
  })
})
...
describe('ispatternInt', () => {
  test('valid pattern', () => {
    expect(ispatternInt(Number('1120'))).toBe(true)
    expect(ispatternInt(Number('02'))).toBe(true)
  })
  test('invalid pattern', () => {
    expect(ispatternInt(Number('0'))).toBe(false)
  })
})
...
describe('getIdParts', () => {
  ...
  test('invalid patterns', () => {
    expect(getIdParts('placemats-4')).toEqual({
      appCode: null,
      categoryId: 'invalid',
      colorId: null,
      colorNumber: 0,
      errors: [
        { message: 'Invalid pattern prefix or pattern id.', part: 'prefix' },
        { part: 'colorId', message: 'Missing valid Color Id.' },
      ],
      input: 'placemats-4',
      invalidId: true,
      itemId: null,
      patternId: 'placemats-4',
      patternInt: 4,
      patternPrefix: 'placemats',
  ...
  test('valid patterns', () => {
    ...
    expect(getIdParts('PF7006')).toMatchObject({
      appCode: 'rg',
      categoryId: 'any',
      colorId: null,
      colorNumber: 0,
...
describe('getItemParts', () => {
  ...
  test('invalid patterns', () => {
    ...
    expect(getItemParts('placemats-6')).toEqual(['placemats', '6'])
  })
})
describe('getPartsInfo', () => {
  test('valid patterns', () => {
    ...
    expect(getPartsInfo('9702')).toEqual({
      input: '9702',
      patternInt: 9702,
      patternPrefix: null,
      patternSeparator: false,
...
describe('getPatternIdInfo', () => {
  test('invalid patterns', () => {
    ...
    expect(getPatternIdInfo('890045')).toEqual({
      appCode: 'rg',
      categoryId: 'textile',
      input: '890045',
      patternInt: 890045,
...
describe('toArchiveId', () => {
  test('basic patterns', () => {
    ...
    expect(toArchiveId('pf710014-21')).toBe('AQ-NNBY-N')
    ...
    expect(toArchiveId('pf710014-21', true)).toBe('A1Q-NNBY-21')
...
describe('fromArchiveId', () => {
  test('basic patterns', () => {
    ...
    expect(fromArchiveId('A1R-NNBY-21').itemId).toBe('rg710014-21')
...
describe('getItemIdInfo', () => {
  ...
  test('invalid', () => {
    expect(getItemIdInfo('placemats-6')).toMatchObject({
      appCode: null,
      categoryId: 'invalid',
      archiveId: null,
      invalidId: true,
...
describe('patternIsTextile', () => {
  ...
  test('3 numbers invalid', () => {
    ...
    expect(patternIsTextile('pf001')).toBe(false)
  })
  test('removes dl prefix', () => {
    expect(patternIsTextile('dl1001')).toBe(false)
  })
  test('4 numbers or more', () => {
    expect(patternIsTextile('dl1011')).toBe(false)
    expect(patternIsTextile('00100')).toBe(false)
    expect(patternIsTextile('08100')).toBe(false)
    expect(patternIsTextile('9702')).toBe(true)
    expect(patternIsTextile('01094402')).toBe(true)
    expect(patternIsTextile('710023')).toBe(true)
  })
})
...
describe('isPatternPart', () => {
  ...
  test('invalid patterns', () => {
    expect(isPatternPart('730006b')).toBe(false)
    expect(isPatternPart('710014a')).toBe(false)
  })
})

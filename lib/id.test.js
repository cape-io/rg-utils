import {
  ...
  hasValidIdFormat, hasValidPatternFormat,
...
describe('getPartsInfo', () => {
  test('valid patterns', () => {
    expect(getPartsInfo('dl10002').patternSeparator).toBe(false)
...
describe('getPatternIdInfo', () => {
  test('invalid patterns', () => {
    ...
    expect(getPatternIdInfo('r-')).toEqual({
      ...
      prefixNumber: 15,
      errors: [
        { part: 'prefix', message: 'Invalid pattern prefix.' },
...
describe('numberToPrefix', () => {
  test('basic patterns', () => {
    expect(numberToPrefix(3)).toEqual({ patternPrefix: 'dl', patternSeparator: true })
    expect(numberToPrefix(19)).toEqual({ patternPrefix: 'dl', patternSeparator: false })
  })
})
describe('toArchiveId', () => {
  test('basic patterns', () => {
    expect(toArchiveId('p-710014-01')).toBe('A1-NNBY-1')
    expect(toArchiveId('dl5002-16|17')).toBe('AK-4WA-40H')
    expect(toArchiveId('dl-5002-16|17')).toBe('A3-4WA-40H')
...
describe('fromArchiveId', () => {
  test('basic patterns', () => {
    ...
    expect(fromArchiveId('Ak-4wA-40H').itemId).toBe('dl5002-16|17')
    expect(fromArchiveId('A3-4wA-40H').itemId).toBe('dl-5002-16|17')
  ...
  test('validIds', () => {
    validIds.forEach((id) => {
      ...
      expect(fromArchiveId(archiveId).itemId).toBe(itemId)
      // test(`${itemId} ${archiveId}`, () => {
...
describe('getItemIdParts', () => {
  ...
  test('invalid', () => {
    expect(getItemIdParts('p 40')).toEqual({
      ...
      patternSeparator: true,
      errors: [
        { part: 'colorId', message: 'Missing colorId' },
      ],
    })
    expect(getItemIdParts('toRrent_sAnd-ebOny')).toEqual({
      ...
      errors: [
        ...
        { part: 'pattern', message: 'Missing a pattern number.' },
        { part: 'colorId', message: 'Missing colorId' },
        { part: 'otherId', message: 'Item id has extra information at the end.' },
      ],
    })

...
export function searchHashParse({ categoryCodes, fieldCodes }, filterHash = ANY_CAT) {
  if (!filterHash) return SEARCH_STUB
  if (filterHash === '[...]' || filterHash === ANY_CAT) return SEARCH_STUB
  if (_.has(filterHash, categoryCodes)) {
    return addFieldFilters(SEARCH_STUB, categoryCodes[filterHash])
  }
...
export const getNormalImg = _.find({ keyword: NORMAL_IMG })
export const normalImgPath = (x) => (getNormalImg(x.images) ? itemImgPath(x, NORMAL_IMG) : null)

...
describe('searchHashParse', () => {
  const info = { fieldCodes, categoryCodes }
  test('category hashes', () => {
    expect(searchHashParse(info, 'textile')).toEqual({
      filters: {
        cg: ['tl'],
      },
    })
    expect(searchHashParse(info, 'leather')).toEqual({
      filters: {
        cg: ['lr'],
      },
    })
    expect(searchHashParse(info, 'passementerie')).toEqual({
      filters: {
        cg: ['pr'],
      },
    })
  ...
  test('search strings', () => {
    expect(searchHashParse(info, 'foo')).toEqual({
      filters: {},
      query: 'foo',
    })
  })

import {
  ...
  getDisplaySort,
  searchHashEncode, searchHashPrep, searchHashParse,
  queryEncode,
} from './path.js'
import hashInfo from './fields.js'
...
describe('searchHashEncode', () => {
  test('itemId + filters', () => {
    ...
    expect(searchHashEncode(hashInfo, search)).toBe('cgtl_sr91009-01')
    const search2 = {
      itemId: '91009-01',
      filters: { categoryId: 'textile' },
    }
    expect(searchHashEncode(hashInfo, search2)).toBe('cgtl_sr91009-01')
  })
  test('query over itemId', () => {
    ...
    expect(searchHashEncode(hashInfo, search)).toBe('91009-02')
  })
  test('only query string', () => {
    const search = {
      query: 'foo',
    }
    expect(searchHashEncode(hashInfo, search)).toBe('foo')
  })
  test('only query string. sr prefix last', () => {
    ...
    expect(searchHashEncode(hashInfo, search)).toBe('cgtl_srfoo')
  })
})

describe('searchHashParse', () => {
  test('category hashes', () => {
    expect(searchHashParse(hashInfo, 'textile')).toEqual({
      filters: {
        // cg: ['tl'],
        categoryId: 'textile',
      },
    })
    expect(searchHashParse(hashInfo, 'leather')).toEqual({
      filters: {
        // cg: ['lr'],
        categoryId: 'leather',
      },
    })
    expect(searchHashParse(hashInfo, 'passementerie')).toEqual({
      filters: {
        // cg: ['pr'],
        categoryId: 'trim',
      },
  ...
  test('search strings', () => {
    expect(searchHashParse(hashInfo, 'foo')).toEqual({
      filters: {},
      query: 'foo',
    })
    expect(searchHashParse(hashInfo, 'linen')).toEqual({
      filters: {},
      query: 'linen',
    })
    expect(searchHashParse(hashInfo, 'sring')).toEqual({
      filters: {},
      query: 'sring',
    })
  })
  test('simpleSearch', () => {
    expect(searchHashParse(hashInfo, '^white')).toEqual({
      filters: {
  ...
  test('valid colorId', () => {
    expect(searchHashParse(hashInfo, '91009-02')).toEqual({
      filters: {},
      itemId: '91009-02',
      query: '91009-02',
    })
  ...
  test('filterCodes', () => {
    expect(searchHashParse(hashInfo, 'cgtl_srfoo')).toEqual({
      filters: {
        matches: { categoryId: 'textile' },
      },
      query: 'foo',
    })
    expect(searchHashParse(hashInfo, 'cgtl_csgdgn')).toEqual({
      filters: {
        colors: ['gold', 'green'],
        matches: { categoryId: 'textile' },
      },
    })
    expect(searchHashParse(hashInfo, 'cgtl_csgdgn_ctln_dnsp_srcottage')).toEqual({
      filters: {
        colors: ['gold', 'green'],
        contents: ['linen'],
        designs: ['stripe'],

...
function getSimpleSearch() {
  const cats = _.fromPairs(
    fieldById.category.options.map(({ id, slug }) => ([slug || id, { categoryId: id }])),
  )
...
export function addSearchFilter(searchHash, filterId, valueId) {
  const search = searchHashParse(searchHash)
  const searchFilters = _.getOr([], [FILTERS, filterId], search)
  searchFilters.push(valueId)
  const nextSearch = _.set([FILTERS, filterId], searchFilters, search)
  const nextHash = searchHashEncode(nextSearch)
  if (searchHash === nextHash) {
    // console.error('ERR addSearchFilter invalid filter and/or value')
    return null
  }
  return nextHash
}

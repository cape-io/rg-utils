...
import { getIdParts, isItemParts } from './id.js'
...
export const categoryCodes = getCategoryCodes(fieldById.categoryId)
...
function getSimpleSearches() {
  const cats = fieldById.categoryId.options
...
// indexed by searchHash
export const simpleSearch = _.fromPairs(simpleSearches)
...
export function searchHashEncodeRaw(info) {
  if (_.isEmpty(info)) return ANY_CAT
  const {
...
const getSearchAliasIndex = _.flow(
  _.filter(([x, s]) => !s.matches),
  _.map(([id, filters]) => ([searchHashEncodeRaw({ filters }), id])),
  _.fromPairs,
  _.unset('any'),
)

// key of encoded hash, value is the simpleSearch alias.
...
function getSimpleSearch(filterHash) {
  const search = _.set(FILTERS, _.get(filterHash, simpleSearch), SEARCH_STUB)
  search.simpleSearch = filterHash
  return search
}
...
export function searchHashParse(searchHash, toIds = true) {
  ...
  if (_.has(filterHash, simpleSearch)) {
    return getSimpleSearch(filterHash)
  }
  if (filterHash in searchAliasIndex) {
    return getSimpleSearch(getSearchAlias(filterHash))
  }
  // Check if it is a valid itemId attempt
  const itemIdParts = getIdParts(filterHash)
  if (isItemParts(itemIdParts)) {
    return {
      ...SEARCH_STUB,
      itemId: itemIdParts.itemId,
      categoryId: itemIdParts.categoryId,
      [SEARCH_Q_KEY]: filterHash,
      prefix: true,
    }
  }
  if (filterHash[0] === '^') {
